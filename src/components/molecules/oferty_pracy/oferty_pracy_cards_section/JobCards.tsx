...
export default function JobCards() {
    ...
    const submit = async () => {
        ...
        const localisation = (document.getElementById("localisation-input")! as HTMLInputElement).value
        const filter = (document.getElementById("filter-input")! as HTMLInputElement).value

        const response = await axios.post(`${uri}/getJobsByFilter`, { keyWord: keyWord, localisation: localisation, filter: filter })
    ...
    return (
            <>
                <button className={styles.submit__button} type={'submit'} onClick={submit}>
                    Submit
                </button>
                            ...
                            {card.map((i: any, o: any) => {
                                return (
                                    <JobCardComponent
                                        ...
                                        position={i.position}
                                        whatAreWeLookingFor={
                                            i.whatAreWeLookingFor.slice(0, 4).map((i: any, o: any) => {
                                                return i
                                            })
                                        }
